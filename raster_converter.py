...
def raster_convert(image_path, resize=False, outwidth=64):
    ...
    index_data = [
        get_color_index(closestcolor(pixel, rgbpalette), palettedict)
    ...
    index_array = [
        index_data[i : i + outwidth] for i in range(0, len(index_data), outwidth)
    ]
...
def hexagonal_convert(image_path, outwidth=64):
    ...
    for y in range(outheight):
        ...
        for x in range(outwidth):
            ...
            color = np.average(
                imgarray[r[0][1] : r[3][1], r[0][0] : r[1][0]], axis=(0, 1)
            )
